...
// Décode le payload d'un JWT sans vérification de signature (client-side uniquement).
...
// Fusionne le payload JWT dans le user pour garantir que is_super_admin, tenant_id etc. sont toujours à jour.
function getStoredSession() {
  ...
  // Le payload JWT fait autorité sur is_super_admin et is_admin
  const mergedUser   = { ...storedUser, ...jwtPayload };
  const storedTenant = tenant ? JSON.parse(tenant) : null;

  // Si le tenant est encodé dans le JWT mais pas en session, on le reconstruit
